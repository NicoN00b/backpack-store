...
public class Sql2oSurfpackDao implements BackPackDao{
    ...
    public List getAll() {
        try(Connection con = sql2o.open()) {
            return con.createQuery("SELECT * FROM backpacks WHERE type = 'surfpack'")
                    .throwOnMappingFailure(false)
                    .executeAndFetch(Surfpack.class);
        }
    }

    @Override
    public Surfpack findById(int id) {
        try(Connection con = sql2o.open()){
            return con.createQuery("SELECT * FROM backpacks WHERE id = :id  AND type = 'surfpack'")
                    .addParameter("id", id)
                    .throwOnMappingFailure(false)
                    .executeAndFetchFirst(Surfpack.class);
        }
    }
    ...
    public void update(Backpack backpack, String newBrand, String newModel, String newDescription, int newWaterResistance, int newDurability, int newProductId, double newPrice){
        String sql = "UPDATE backpacks SET (brand, model, description, waterResistance, durability, productId, price) = (:brand, :model, :description, :waterResistance, :durability, :productId, :price) WHERE id = :id AND type = 'surfpack'";
